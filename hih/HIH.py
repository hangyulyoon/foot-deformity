...
class HIH_head(nn.Module):
    def __init__(self,config):
        ...
        target_c = config.target_o # default target_o = 384
...
class Hourglass(nn.Module):
    ...
    def _init_layers(self,n,f):
        ...
        if n > 1:
            self._init_layers(n - 1, f) # create self.res+str(n-1)_1, self.pool+str(n-1)_1, self.res+str(n-1)_2 for each n>1
        else:
...
class StackedHourGlass(nn.Module):
    def __init__(self,config):
        ...
        self.pre_conv = nn.Sequential(
           ConvBNReLu(1,config.num_feature//4,7,2),
           Residual(config.num_feature // 4, config.num_feature // 2),
           nn.MaxPool2d(2,2), # can remove this if we want to increase the size 
        ...
        self.offset_func = eval(config.head_type.upper() + '_head') if config.head_type.upper() != 'WOO' else None # default: HIH_head    
        ...
        if config.refine_heatmap : 
            self.refine_func = nn.Sequential(
                nn.Conv2d(config.num_landmarks, config.num_landmarks*4, 7,1,3), 
                nn.BatchNorm2d(config.num_landmarks*4),
                nn.ReLU(inplace=True),
                #nn.Conv2d(config.num_landmarks*4, config.num_landmarks*4, 7,1,3), 
                #nn.BatchNorm2d(config.num_landmarks*4),
                #nn.ReLU(inplace=True),
                nn.Conv2d(config.num_landmarks*4, config.num_landmarks, 7,1,3),
            )
    ...
    def forward(self,x):
        ...
        for i in range(self.config.num_stack):
            ...
            x = x+merge_feature+merge_preds # (4, 256, 128, 128)
        ...
        if self.refine_func is not None : 
            ## If use gumbel_softmax
            # B,C,H,W = out_preds[-1].shape 
            # h = out_preds[-1].view(B,C,-1) # B, C, HW
            # h = F.gumbel_softmax(h,hard=True, dim=-1)
            # h = h.view(B,C,H,W) 

            ## If do not use gumbel_softmax
            h = out_preds[-1]
            out_preds.append(F.relu(self.refine_func(h)*self.r + out_preds[-1]))
